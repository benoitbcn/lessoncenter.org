// Génère du contenu variable à chaque build
const fs = require('fs'); const path = require('path');
const out = path.join(__dirname, 'dist');
fs.mkdirSync(out, {recursive:true}); fs.mkdirSync(path.join(out,'assets'), {recursive:true});
const rnd=(a,b)=>Math.floor(Math.random()*(b-a+1))+a;
const now=new Date().toISOString();
const para=rnd(6,22), pad=rnd(500,12000);
function paraHtml(n){let s=''; for(let i=0;i<n;i++){s+=`<p>Mise à jour auto. Build: ${now}. Bloc ${i+1}.</p>\n`;} return s;}
const html=`<!doctype html><html lang='fr'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>lessoncenter.org — dynamique</title><link rel='stylesheet' href='/assets/style.css'></head><body><h1>Auto-build</h1><p>Dernière génération: ${now}</p><p>Paragraphes: ${para} — Padding: ~${pad} octets</p>${paraHtml(para)}<!-- ${'x'.repeat(pad)} --></body></html>`;
